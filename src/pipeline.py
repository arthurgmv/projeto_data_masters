...
def run_task(script_name, step_name):
    ...
    try:
        subprocess.run(["py", f"src/{script_name}"], check=True)
        ...
        try:
            ...
            logger.error(f"{Fore.RED}❌ Erro de Sistema: Não foi possível encontrar o interpretador Python.{Style.RESET_ALL}")
            return False
